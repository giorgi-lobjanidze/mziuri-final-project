  ...
  return (
    <div className='filter-section'>
      ...
      <div className='filter-group'>
        ...
        {categories.map(i => (
          <div key={i} className='filter-item' onClick={() => toggleCategory(i)}>
            <span>{i}</span>
            <span>(12)</span>
          </div>
        ))}
      ...
      <div className='filter-group'>
        ...
        <label><input type='checkbox' onChange={() => toggleAvailability('instock')} /> In Stock (9)</label>
        <label><input type='checkbox' onChange={() => toggleAvailability('outofstock')} /> Out Of Stock (6)</label>
      </div>

      <div className='filter-group'>
        <div className='filter-group-header'>
          <h4>PRICE</h4>
          <span onClick={() => setFilters(prev => ({ ...prev, priceFrom: '', priceTo: '' }))}>RESET</span>
        </div>
        <p>The Highest Price Is <span className='price-highlight'>$200.00</span></p>
        <div className='price-inputs'>
          <input type='number' placeholder='From' value={priceFrom} onChange={e => { setPriceFrom(e.target.value); setFilters(prev => ({ ...prev, priceFrom: e.target.value })) }} />
